...
export default function MarketDataPanel() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => { void loadQuotes(symbols, true); }, 30_000);
  ...
  const chartPath = useMemo(() => {
    ...
    const min = Math.min(...chartPoints.map((c) => c.close));
    const max = Math.max(...chartPoints.map((c) => c.close));
    const spread = max - min || 1;
    return chartPoints.map((c, i) => {
      const x = (i / (chartPoints.length - 1)) * 100;
      const y = 92 - ((c.close - min) / spread) * 78;
  ...
  return (
    <section className="mt-4 grid gap-4 xl:grid-cols-[1.7fr_1fr]">
      <div className="rounded border border-[#1b2532] bg-[#0c1118] p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-lg font-semibold">Live quote monitor</div>
          </div>
          <div className="flex items-center gap-2 text-[10px] text-[#7f8da1]">
            <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#5eead4]" /> AUTO REFRESH 30S
            <button onClick={() => void loadQuotes(symbols)} className="rounded border border-[#263444] px-2 py-1 text-[#9aa8ba] hover:border-[#5eead4] hover:text-[#5eead4]">REFRESH</button>
          </div>
        </div>

        <form onSubmit={addSymbol} className="mb-4 flex gap-2">
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Add symbol, e.g. META, INFY, BTC-USD" className="min-w-0 flex-1 rounded border border-[#263444] bg-[#101722] px-3 py-2 text-sm outline-none placeholder:text-[#526176] focus:border-[#5eead4]" />
          <button className="rounded border border-[#5eead4] px-3 text-xs font-bold text-[#5eead4] hover:bg-[#102522]">ADD</button>
        </form>

        {error ? <div className="mb-3 rounded border border-[#4a2930] bg-[#171016] p-3 text-sm text-[#f0a8b2]">{error}</div> : null}
        <div className="overflow-x-auto">
          <table className="w-full min-w-[680px] text-left text-sm">
            <thead className="border-b border-[#1b2532] text-[10px] uppercase tracking-[0.14em] text-[#7f8da1]"><tr><th className="pb-3">Symbol</th><th className="pb-3">Price</th><th className="pb-3">Change</th><th className="pb-3">Day range</th><th className="pb-3">Volume</th><th className="pb-3">Status</th></tr></thead>
            <tbody>
              {loading && !quotes.length ? Array.from({ length: 6 }).map((_, i) => <tr key={i} className="border-b border-[#151e29]"><td colSpan={6} className="py-4 text-[#526176]">Loading market data…</td></tr>) : quotes.map((quote) => {
                ...
                const isSelected = quote.symbol === active?.symbol;
                return <tr key={quote.symbol} onClick={() => setSelected(quote.symbol)} className={`cursor-pointer border-b border-[#151e29] transition-colors hover:bg-[#101722] ${isSelected ? "bg-[#101722]" : ""}`}>
      ...
      <div className="rounded border border-[#1b2532] bg-[#0c1118] p-5">
        <div className="flex items-center justify-between"><div className="text-xs font-bold tracking-[0.16em] text-[#5eead4]">QUOTE DETAIL</div>{active ? <span className="text-[10px] text-[#617086]">{active.currency}</span> : null}</div>
        {active ? <>
          ...
          <div className="mt-5 h-40 rounded border border-[#1b2532] bg-[#101722] p-2">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 text-xs">{[["Previous close", active.previousClose], ["Day high", active.dayHigh], ["Day low", active.dayLow], ["Volume", active.volume === null ? null : compact.format(active.volume)]].map(([label, value]) => <div key={label as string} className="rounded border border-[#1b2532] bg-[#101722] p-3"><div className="text-[#617086]">{label}</div><div className="mt-1 font-medium tabular-nums">{typeof value === "number" ? value.toFixed(2) : value ?? "—"}</div></div>)}</div>
          <div className="mt-4 flex justify-between border-t border-[#1b2532] pt-4 text-[10px] text-[#617086]"><span>{active.provider} · {active.freshness}</span><span>{new Date(active.timestamp).toLocaleTimeString()}</span></div>
