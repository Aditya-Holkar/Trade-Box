...
export default function Home() {
  ...
  return <main className="min-h-screen p-4 md:p-6"><div className="mx-auto max-w-[1600px]">
    <header className="mb-5 flex flex-wrap items-center gap-4 border-b border-[#1b2532] pb-4">
      ...
      <div className="rounded border border-[#23413d] bg-[#0c1516] px-3 py-2 text-xs text-[#5eead4]">PHASE 6 · PRICE STRUCTURE</div>
    </header>

    <section className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
      <div className="rounded border border-[#1b2532] bg-[#0c1118] p-5"><div className="text-xs font-bold tracking-[0.16em] text-[#5eead4]">DRAWING & PRICE STRUCTURE</div><h1 className="mt-2 text-2xl font-semibold">Map the levels around price.</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-[#9aa8ba]">Phase 6 adds automatic swing structure, support and resistance clustering, and Fibonacci retracement levels. These levels are derived from the selected instrument's recent OHLCV history and update when the symbol changes.</p><div className="mt-6 grid gap-2 sm:grid-cols-4">{[["SWINGS", "Confirmed highs/lows"], ["S/R", "Nearest levels"], ["FIB", "0–100% retracement"], ["DYNAMIC", "Recalculates per symbol"]].map(([k,v]) => <div key={k} className="rounded border border-[#1b2532] bg-[#101722] p-3"><div className="text-[10px] tracking-widest text-[#7f8da1]">{k}</div><div className="mt-1 text-sm">{v}</div></div>)}</div></div>
      <div className="rounded border border-[#1b2532] bg-[#0c1118] p-5"><div className="text-xs font-bold tracking-[0.16em] text-[#5eead4]">PHASE 6 CHECKLIST</div><div className="mt-4 space-y-3 text-sm">{["Swing-high / swing-low detection", "Support level clustering", "Resistance level clustering", "Fibonacci retracement", "Dynamic symbol updates", "Chart-ready structure engine", "Interactive drawing tools next"].map((item) => <div key={item} className="flex items-center gap-3"><span className="text-[#5eead4]">✓</span><span>{item}</span></div>)}</div></div>
    </section>
