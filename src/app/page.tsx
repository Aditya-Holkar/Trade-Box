    ...
    <main className="min-h-screen p-4 md:p-6">
      <div className="mx-auto max-w-[1600px]">
        <header className="mb-5 flex flex-wrap items-center gap-4 border-b border-[#1b2532] pb-4">
          ...
          <div className="ml-auto flex min-w-[280px] max-w-xl flex-1 items-center rounded border border-[#1b2532] bg-[#0c1118] px-3 py-2 text-sm text-[#7f8da1]"><span className="mr-2 text-[#5eead4]">⌘K</span>Search symbols, companies, markets...</div>
          <div className="rounded border border-[#1b2532] px-3 py-2 text-xs text-[#7f8da1]">PHASE 1</div>
        </header>

        <section className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
          <div className="rounded border border-[#1b2532] bg-[#0c1118] p-5"><div className="text-xs font-bold tracking-[0.16em] text-[#5eead4]">MARKET DATA ENGINE</div><h1 className="mt-2 text-2xl font-semibold">A reliable data boundary for Trade Box.</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-[#9aa8ba]">Quotes and historical OHLCV now flow through a normalized server-side provider layer. The UI does not call market vendors directly.</p><div className="mt-6 grid gap-2 sm:grid-cols-3">{[["QUOTE", "Normalized quote model"], ["HISTORY", "OHLCV candles"], ["CACHE", "30s / 60s server cache"]].map(([k,v]) => <div key={k} className="rounded border border-[#1b2532] bg-[#101722] p-3"><div className="text-[10px] tracking-widest text-[#7f8da1]">{k}</div><div className="mt-1 text-sm">{v}</div></div>)}</div></div>
          <div className="rounded border border-[#1b2532] bg-[#0c1118] p-5"><div className="text-xs font-bold tracking-[0.16em] text-[#5eead4]">PHASE 1 CHECKLIST</div><div className="mt-4 space-y-3 text-sm">{["Normalized market-data contracts", "Yahoo Finance provider", "Provider-independent service", "Server-side caching", "Quote API endpoint", "Historical OHLCV API endpoint"].map((item) => <div key={item} className="flex items-center gap-3"><span className="text-[#5eead4]">✓</span><span>{item}</span></div>)}</div></div>
        </section>
